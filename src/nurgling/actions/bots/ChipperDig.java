...
public class ChipperDig implements Action {
    ...
    public Results run(NGameUI gui) throws InterruptedException {
        ...
        if(prop.plateu) {
            if (!new Equip(new NAlias("Metal Shovel")).run(gui).IsSuccess())
                return Results.ERROR("Equipment not found: " + "Metal Shovel");
        }
        else {
            ...
            while (!(bumlings = Finder.findGobs(insa.getRCArea(), pattern)).isEmpty()) {
                ...
                while (Finder.findGob(bumling.id) != null) {
                    ...
                    if (prop.tool.equals("Pickaxe")) {
                        ...
                        if (item != null && NParser.checkName(((NGItem) item.item).name(), "Metal Shovel"))
        ...
        {
            Coord2d mountain = NUtils.findSand(insa.getRCArea());
            if(mountain == null)
            {
