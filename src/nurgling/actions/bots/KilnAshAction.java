...
public class KilnAshAction implements Action {
    ...
    public Results run(NGameUI gui) throws InterruptedException {
        ...
        NArea toFire = NArea.findSpec(Specialisation.SpecName.toFire.toString());
        ...
        //for(NArea area : NArea.findAllIn(new NAlias("Block"))) {
            for (Gob sm : Finder.findGobs(toFire, new NAlias(new ArrayList<>(Context.contcaps.keySet())))) {
                Container cand = new Container();
                cand.gob = sm;
                cand.cap = Context.contcaps.get(cand.gob.ngob.name);
                cand.initattr(Container.Space.class);
                cand.initattr(Container.TargetItems.class);
                cand.getattr(Container.TargetItems.class).addTarget("Block");
        ...
        while(res == null || res.IsSuccess()) {
            ...
            res = new FillContainersFromAreas(containers, new NAlias("Block"), icontext).run(gui);
